...
void ProcessKey(void)
{
	if (GetAsyncKeyState('A'))
		ZCamera::GetInstance()->MoveLocalZ(0.5f);	// 카메라 전진!
	if (GetAsyncKeyState('Z'))
		ZCamera::GetInstance()->MoveLocalZ(-0.5f);	// 카메라 후진!
}
...
VOID Animate()
{
	...
	D3DXMATRIXA16	m;
	D3DXMATRIXA16	*pView;
	pView = ZCamera::GetInstance()->GetViewMatrix();	// 카메라 클래스로부터 행렬정보를 얻는다.
	m = *pView * g_matProj;				// World좌표를 얻기위해서 View * Proj행렬을 계산한다.
	if (!g_bLockFrustum)
		ZFrustum::GetInstance()->Make(&m);	// View*Proj행렬로 Frustum을 만든다.
...
VOID Render()
{
	...
	{
		g_pTerrain->Draw(ZFrustum::GetInstance());
		if (!g_bHideFrustum)
